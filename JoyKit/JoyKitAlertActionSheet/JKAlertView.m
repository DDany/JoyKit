...
@interface JKAlertView () <UIAlertViewDelegate>
...
{
	JKAlertView* alert = [[self alloc] initWithTitle:title message:message
...
+ (void)showAlertWithTitle:(NSString *)title message:(NSString *)message
            ...
            onButtonTapped:(void(^)(JKAlertView* alert, NSInteger buttonIndex))handler
{
   ...
   timeoutMessageFormat:(NSString*)countDownMessageFormat
{
    ...
    JKAlertViewButtonHandler finalHandler = [self.buttonHandler copy];
    self.buttonHandler = ^(JKAlertView* bhAlert, NSInteger bhButtonIndex) {
